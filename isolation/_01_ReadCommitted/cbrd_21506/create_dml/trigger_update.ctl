...
C1: create table t(i int primary key,j char(10),k int not null);
C1: insert into t select rownum,rownum,rownum from db_root connect by level<=5;
C1: CREATE TRIGGER trigger1 after UPDATE ON t  EXECUTE insert into t values(-1,-1,-1);
C1: COMMIT;
...
C3: update t set k=(select -10 from t limit 1) where k=5;
...
/* verification */
C1: select k from t ignore index (a) where k > -999 order by 1;
C1: select k from t force index (a) where k > -999 order by 1;
MC: wait until C1 ready;
