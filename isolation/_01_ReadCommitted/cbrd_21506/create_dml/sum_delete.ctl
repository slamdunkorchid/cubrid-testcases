...
C3: delete from t where k > (select sum(k) from t where k between 1 and 3);
...
/* verification */
C1: select sum(set{k}) into :s from t ignore index (a) where k > -999 and k < 999 order by 1;
C1: select sum(set{k}) into :i from t force index (a) where k > -999 and k< 999 order by 1;
