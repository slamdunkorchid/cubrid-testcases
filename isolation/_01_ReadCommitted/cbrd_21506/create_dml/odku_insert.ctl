...
C1: create table t(i int primary key,j char(10),k int unique);
...
C2: create index a on t(k,j) with online;
MC: wait until C2 blocked;

C3: insert into t values (-1,-1,1) on duplicate key update k=k-10;
...
/* verification */
C1: select sum({k}) into :s from t ignore index (a) where k > -999 order by 1;
C1: select sum({k}) into :i from t force index (a) where k > -999 order by 1;
