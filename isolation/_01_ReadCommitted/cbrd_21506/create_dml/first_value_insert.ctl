...
C3: insert into t select -1,-1,FIRST_VALUE(i) over(partition by k order by i) from t order by 3 desc limit 1;
...
/* verification */
C1: select sum({k}) into :s from t ignore index (a) where k > 0 order by 1;
C1: select sum({k}) into :i from t force index (a) where k > 0 order by 1;
