...
C1: insert into t values(-1,-1,null);
...
C3: delete from t where k in (select max(k) from t);
MC: wait until C3 blocked;


C4: insert into t values(16,16,16);
...
/* verification */
C1: select sum(set{k}) from t ignore index (a) where k>0   order by 1;
C1: select sum(set{k}) from t force index (a) where k >0  order by 1;
MC: wait until C1 ready;
